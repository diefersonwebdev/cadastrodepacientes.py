...
class MainWindow(tkinter.Tk):
    ...
    def __init__(self):
        ...
        ttk.Label(self.mainframe, text='<span class="censurado">[*****]</span>: ').grid(row=2, column=1, sticky=tkinter.E)
        self.span="censurado">[*****]</span> = tkinter.StringVar()
        self.masculino = ttk.Radiobutton(self.mainframe, text='Masculino', variable=self.<span class="censurado">[*****]</span>, value='Masculino')
        self.feminino = ttk.Radiobutton(self.mainframe, text='Feminino', variable=self.<span class="censurado">[*****]</span>, value='Feminino')
    ...
    def connect(self, filename):
        ...
        if create:
            ...
            cursor.execute("CREATE TABLE pacientes ("
                "id INTEGER PRIMARY KEY AUTOINCREMENT UNIQUE NOT NULL, "
                "nome TEXT NOT NULL, "
                "<span class="censurado">[*****]</span> TEXT, "
                "cartao TEXT, "
                "dia_nasc TEXT, "
                "mes_nasc TEXT, "
                "ano_nasc TEXT, "
    ...
    def blank(self, *ignore):
        self.nome.set('')
        self.<span class="censurado">[*****]</span>.set(None)
        self.cartao.set('')
        self.dia_nasc.set('01')
        self.mes_nasc.set('Janeiro')
        self.ano_nasc.set('1950')
    ...
    def abrir_id(self, *ignore):
        cursor = self.db.cursor()
        cursor.execute("SELECT pacientes.nome, pacientes.<span class="censurado">[*****]</span>, pacientes.cartao, pacientes.dia_nasc, pacientes.mes_nasc, "
        ...
        nome, <span class="censurado">[*****]</span>, cartao, dia_nasc, mes_nasc, ano_nasc, endereco, cidade, plano, estado, cep, telefone, celular = cursor.fetchone()
        self.nome.set(nome)
        self.<span class="censurado">[*****]</span>.set(<span class="censurado">[*****]</span>)
    ...
    def salvar(self, *ignore):
        ...
        if not nome:
            ...
            return
        <span class="censurado">[*****]</span> = self.<span class="censurado">[*****]</span>.get()
        ...
        if identity is None:
            ...
            cursor.execute("INSERT INTO pacientes "
                    "(nome, <span class="censurado">[*****]</span>, cartao, dia_nasc, mes_nasc, ano_nasc, endereco, cidade, estado, cep, telefone, celular, plano_id) "
                    "VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
                    (nome, <span class="censurado">[*****]</span>, cartao, dia_nasc, mes_nasc, ano_nasc, endereco, cidade,
                        estado, cep, telefone, celular, plano_id))
            self.db.commit()
        else:
            cursor = self.db.cursor()
            cursor.execute("UPDATE pacientes SET nome=:nome, <span class="censurado">[*****]</span>=:<span class="censurado">[*****]</span>, cartao=:cartao, dia_nasc=:dia_nasc, mes_nasc=:mes_nasc, "
    ...
    def importar_db(self, *ignore):
        ...
        for element in tree.findall("pac"):
            try:
                ...
                <span class="censurado">[*****]</span> = element.get("<span class="censurado">[*****]</span>")
    ...
    def exportar_db(self, *ignore):
        ...
        try:
            with open(fileName, mode="w", encoding="UTF-8") as fh:
                ...
                fh.write("<pacientes>\n")
                for record in cursor:
                    fh.write('<pac <span class="censurado">[*****]</span>={0} cartao={1} '
